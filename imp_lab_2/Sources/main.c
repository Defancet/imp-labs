...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: (Maksim Kalutski, xkalut00)               */
/*                                                                            */
/******************************************************************************/
...
void PORTB_IRQHandler(void) {
    ...
    if (PORTB_ISFR & BUTTON_UP_MASK) {
		if (GPIOB_PDIR & BUTTON_UP_MASK) {
			flash();
			beep();
		}
    ...
    if (PORTB_ISFR & BUTTON_DOWN_MASK) {
		if (GPIOB_PDIR & BUTTON_DOWN_MASK) {
			flash();
			beep();
		}
		PORTB_ISFR = PORTB_ISFR;
    ...
    if (PORTB_ISFR & BUTTON_LEFT_MASK) {
		if (GPIOB_PDIR & BUTTON_LEFT_MASK) {
			flash();
			beep();
		}
		PORTB_ISFR = PORTB_ISFR;
    ...
    if (PORTB_ISFR & BUTTON_RIGHT_MASK) {
		if (GPIOB_PDIR & BUTTON_RIGHT_MASK) {
			flash();
			beep();
		}
		PORTB_ISFR = PORTB_ISFR;
    ...
    if (PORTB_ISFR & BUTTON_CENT_MASK) {
